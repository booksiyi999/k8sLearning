...
| 交互终端 | 内置 kubectl 终端，只读/写入双模式 + 危险命令确认 + 风险横幅 |
| 后端 | FastAPI + PyYAML 模拟器 + kubectl 集群连接 |
| 前端 | Alpine.js 单页（教学Tab / 练习Tab / 终端Tab / 实操Tab） |

## ✨ 特性

- 📖 **闯关式教学**：每关含六节结构（概念→关键字段→图解→示例YAML→常见错误→学习建议），从 K8s 历史由来到 CKA 考试全覆盖
- 🖥️ **交互式 Kubectl 终端**：只读模式（默认）+ 写入模式切换，命令白名单+注入防护+危险命令确认+集群风险横幅
...
- 🔓 **渐进式章节解锁**：完成上一章1关即解锁下一章，ch00+ch01默认解锁，新手不再被门控卡住
- 🛡️ **终端安全加固**：apply/create/patch等mutating命令纳入DANGEROUS列表，ch21破坏性操作含风险警告
- 🧪 **3329 测试**：含 E2E + 前端逻辑 + QA 攻击(127个) + Playwright前端(15个) + 模拟器集成 + RBAC/NetworkPolicy 行为验证
